...
const generateLocalAnalysis = (area: MapArea, allEvents: CameraEvent[]): React.ReactNode => {
  ...
  if (!baseDescription) {
    ...
    const seed = area.id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) + allEvents.length;
...
export const SiteMap: React.FC<SiteMapProps> = ({ events, chat, onClose, focusCamera }) => {
  ...
  const getAreaClasses = (area: MapArea) => {
    const priority = getPriorityForArea(area.cameras, recentEvents);
    let classes = `absolute flex items-center justify-center p-1 text-center leading-tight transition-all duration-300 cursor-pointer hover:border-cyan-400 hover:bg-cyan-900/30 `;
            ...
            <div className="flex-grow relative w-full h-full overflow-hidden">
                <div className="absolute inset-0 p-2 bg-black/50 map-grid-background">
                    {mapLayoutsForLevel.areas.map((area) => (
                        ...
                        >
                            <span className="text-xs md:text-sm">{area.label}</span>
                        </div>
                    ))}
                </div>
